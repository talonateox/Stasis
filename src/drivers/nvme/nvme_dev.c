...
static int64_t nvme_dev_read(vfs_node_t* node, void* buf, size_t size,
    ...
    void* dma_buffer = pfallocator_request_page();
    if (!dma_buffer) {
        printkf_error("nvme_dev_read(): Failed to allocate DMA buffer\n");
        return -1;
    }
    ...
    while (blocks_read < num_blocks) {
        uint16_t chunk =
            (num_blocks - blocks_read) > 8 ? 8 : (num_blocks - blocks_read);
        ...
        if (nvme_read(ctrl, lba + blocks_read, chunk, dma_buffer) < 0) {
            printkf_error("nvme_dev_read(): NVMe read failed at block %llu\n",
                          lba + blocks_read);
            pfallocator_free_page(dma_buffer);
            return bytes_read;
        }
...
static int64_t nvme_dev_write(vfs_node_t* node, const void* buf, size_t size,
    ...
    void* dma_buffer = pfallocator_request_page();
    if (!dma_buffer) {
        printkf_error("nvme_dev_write(): Failed to allocate DMA buffer\n");
        return -1;
    }
    ...
    while (blocks_written < num_blocks) {
        uint16_t chunk = (num_blocks - blocks_written) > 8
                             ? 8
...
void nvme_register_device(nvme_ctrl_t* ctrl) {
    nvme_device_node_t* dev_node =
        (nvme_device_node_t*)malloc(sizeof(nvme_device_node_t));
    dev_node->ctrl = ctrl;
...
nvme_device_node_t* nvme_get_devices(void) { return device_list; }