...
void* pfallocator_request_page() {
    for (; _g_alloc.page_index < _g_alloc.page_count; _g_alloc.page_index++) {
        uint64_t i = _g_alloc.page_index;
        if (_g_alloc.refcounts[i] != 0) continue;
...
void pfallocator_free_page(void* address) { pfallocator_unref_page(address); }
...
uint64_t pfallocator_get_free_ram() { return free_memory; }

uint64_t pfallocator_get_used_ram() { return used_memory; }
