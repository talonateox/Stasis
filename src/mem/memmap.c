...
size_t memmap_get_entry_count() { return _g_memmap.entry_count; }
...
size_t memmap_get_total() {
    ...
    if (memmap_total_bytes > 0) return memmap_total_bytes;
...
void memmap_print() {
    for (size_t i = 0; i < memmap_get_entry_count(); i++) {
        ...
        printkf("%s: %k%p %k%llu%r\n", entry_names[entry->type], 0x55aaff,
                entry->base, 0xcccc66, entry->length);
    }
